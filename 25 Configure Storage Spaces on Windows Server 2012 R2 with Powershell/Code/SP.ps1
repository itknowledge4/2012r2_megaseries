...
1..6 | ForEach-Object { New-VHD -Path "D:\HDD\[Lab2012R2]-BMC01_$_.vhdx" -SizeBytes 10GB  }
7..14 | ForEach-Object { New-VHD -Path "D:\HDD\[Lab2012R2]-BMC01_$_.vhdx" -SizeBytes 20GB  }
#I renamed the VM without the [] characters since the CmdLets had trouble with them
#After the disks are assigned i will rename it back
1..14 | ForEach-Object {Add-VMHardDiskDrive -VMName 'BMC01' -ControllerType SCSI -Path "D:\HDD\[Lab2012R2]-BMC01_$_.vhdx"}
...
New-StoragePool –FriendlyName StoragePool1 –PhysicalDisks (Get-PhysicalDisk -CanPool $true | sort size | select -first 10) -StorageSubSystemFriendlyName "Storage Spaces*"
#Add the remaining disks as hot spares to the pool
Add-PhysicalDisk -StoragePoolFriendlyName 'StoragePool1' -PhysicalDisks (Get-PhysicalDisk -CanPool $true) -Usage HotSpare
...
Get-PhysicalDisk -StoragePool (Get-StoragePool StoragePool1) | Select-Object FriendlyName,mediatype,size | Sort-Object size | Select-Object -First 6 | Set-PhysicalDisk -MediaType SSD
Get-PhysicalDisk -StoragePool (Get-StoragePool StoragePool1) | Select-Object FriendlyName,mediatype,size | Sort-Object size | Select-Object -Last 8 | Set-PhysicalDisk -MediaType HDD
...
New-VirtualDisk -StoragePoolFriendlyName 'StoragePool1' -FriendlyName 'VirtualDisk3' -ResiliencySettingName 'Mirror' -ProvisioningType Fixed -NumberOfDataCopies 3 -StorageTiers $Fast,$Slow -StorageTierSizes 2GB,4GB
#Create a volume
